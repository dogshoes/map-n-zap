 ...
 **********************************************************************/
  ...
  int getYVal(int x, int y, int dire) {
  	if (dire == 0) return y;
  	else if (dire == 1) return y+1;
  	else if (dire == 2) return y;
  	else return y-1;
  } // getYVal() //
  
  
  int getXVal(int x, int y, int dire) {
  	if (dire==0) return x+1;
  	else if (dire == 1) return x;
  	else if (dire == 2) return x-1;
  	else return x;
  } // getXVal() //
  									 ...
  									 bool GoThroughGrey, bool Shortcut)
  {
  	...
  	robPOINT actualStart; robPOINT actualEnd;
  ...
  void IPathPlanner::QuickCrystallizePath(int IllahStartX, int IllahStartY,
  	...
  	int progress = 1; int currentInflate;
  	/*int inflateLimit;
  			...
  			for (k=0+illahoffsety; ((k<cWorldGridYSize/IGrain) &&
  				...
  				if (ABS(IllahGrid[j][k]) == ABS(currentInflate+1)) {
  					if ((j != 0) && ((IllahGrid[j-1][k] == 0) ||
  						...
  						else IllahGrid[j-1][k] = currentInflate;
  					}  // end if
  					if ((j < cWorldGridXSize/IGrain-1) && 
  							((IllahGrid[j+1][k]==0) ||
  							 (ABS(IllahGrid[j+1][k]) > ABS(currentInflate)))) {
  					...
  					if ((k != 0) && ((IllahGrid[j][k-1]==0) ||
  						...
  						else IllahGrid[j][k-1] = currentInflate;
  					} // endif
  					if ((k < cWorldGridYSize/IGrain-1) && 
  							((IllahGrid[j][k+1]==0) ||
  							(ABS(IllahGrid[j][k+1])>ABS(currentInflate)))) {
  					...
  					if ((j!=0) && (k!=0) && 
  						...
  						else IllahGrid[j-1][k-1] = currentInflate;
  					}
  					if ((j!=0) && (k < cWorldGridYSize/IGrain-1) &&
  						((IllahGrid[j-1][k+1]==0) ||
  						 (ABS(IllahGrid[j-1][k+1])>ABS(currentInflate)))) {
  											...
  											robPOINT end1, robPOINT end2)
  {
  		...
  		lastx = illahj; lasty = illahk;
  		if ((illahj != 0) && (IllahGrid[illahj-1][illahk] == 30000)) {
  			illahj--;
  			//AfxMessageBox("initialleft");
  		} else
  		if ((illahj < cWorldGridXSize/IGrain-1) && (IllahGrid[illahj+1][illahk] == 30000)) {
  			illahj++;
  			//AfxMessageBox("initialright");
  		} else
  		if ((illahk != 0) && (IllahGrid[illahj][illahk-1] == 30000)) {
  			illahk--;
  			//AfxMessageBox("Initialdown");
  		} else 
  		if ((illahk < cWorldGridYSize/IGrain-1) && (IllahGrid[illahj][illahk+1] = 30000)) {
  			illahk++;
