 ...
 **********************************************************************/
  ...
  {
  	if (InsideExecuteZone(point, WorldPointsPerPixel)) return true;
  	if (m_ShowCaption) {
  ...
  bool CLinePath::InsideExecuteZone(robPOINT point, float WorldPointsPerPixel)
  {
  	...
  	if (Distance(PreviousWorldPoint, point) <= WidthObject) return true;
  	while (Position != NULL) {
  		WorldPoint = m_PointList.GetNext(Position);
  		...
  		if ( PointIsBetween(Intersection, WorldPoint, PreviousWorldPoint) ) {
  			if (PerpendicularDistanceFromLine(LineSegment, point) <= WidthObject) return true;
  		} else {
  			if (Distance(point, WorldPoint) <= WidthObject) return true;
  			if (Distance(point, PreviousWorldPoint) <= WidthObject) return true;
  		}
  ...
  {
  	if (Index >= 0) {
  		if (Index == 0) SetEntryPoint(Position);
  		if (Index == m_PointList.GetCount() - 1) SetExitPoint(Position);
  		if (Index >= m_PointList.GetCount()) return;
  ...
  {
  	if (PointListPosition == NULL) return;
  	if (PointListPosition == m_PointList.GetHeadPosition()) SetEntryPoint(Point);
  	if (PointListPosition == m_PointList.GetTailPosition()) SetExitPoint(Point);
