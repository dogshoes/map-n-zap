...
*******************************************************************************/
{
            ...
            (Robot->GetState() == stJoysticking))
    {
        ...
        if (temp == JOYERR_NOERROR)
        {
            ...
            if ((x < cDeadZone && x > 0) || (x > -cDeadZone && x < 0))
            {
                x = 0;
            }

            if ((y < cDeadZone && y > 0) || (y > -cDeadZone && y < 0))
            {
                y = 0;
            }
...
CRobotWorldView::~CRobotWorldView()
{
    ...
    if (m_DlgCyeSample != NULL)
    {
        delete m_DlgCyeSample;
    }
...
void CRobotWorldView::Serialize(CArchive & ar)
{
    ...
    if (!(ar.IsStoring()))
    {
        if (m_DlgFlowZap != NULL)
        {
            ...
            if (m_HomeBase != NULL)
            {
                PositionLabelNames->AddHead(m_HomeBase->GetCaption());
            }
...
BOOL CRobotWorldView::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
    if (pWnd == this && m_SelectedGraphicObject != NULL)
    {
        ...
        if (m_SelectedGraphicObject->SetTrackerCursor(this, nHitTest))
        {
            return TRUE;
        }
    }
...
void CRobotWorldView::SetTestRobotDialogVisible(bool Visible)
{
    ...
    if (Robot != NULL)
    {
        ...
        else
        {
            if (m_DlgTestRobot != NULL)
            {
                m_DlgTestRobot->ShowWindow(SW_HIDE);
            }
        }
    }
}
...
BOOL CRobotWorldView::ShowTip(int state)
{
    ...
    if (m_LaygoNumberClicks <= NextTipClicks)
    {
        m_LaygoNumberClicks++;
    }
    ...
    if (Robot != NULL)
    {
        ...
        case (CLaygo::lgoOccupancyGrid):
            ...
            if (Distance(Position, Origin) > 24 * cTPI)
            {
                Show =  TRUE;
            }
        ...
        case (CLaygo::lgoPositionUncertainty):
            ...
            if (sqrt(SQR(Error.x) + SQR(Error.y)) > 6 * cTPI)
            {
                Show =  TRUE;
            }
        ...
        case (CLaygo::lgoCheckPoint):
            ...
            if (sqrt(SQR(Error.x) + SQR(Error.y)) > 12 * cTPI)
            {
                Show =  TRUE;
            }
        ...
        case (CLaygo::lgoAdditionalCheckPoints):
            ...
            if (Distance(Position, Origin) > 240 * cTPI)
            {
                Show =  TRUE;
            }
        ...
        case (CLaygo::lgoHotPoints):
            if (m_ProbabilityGrid->GetPercentExploredArea() >= 0.5)
            {
                Show =  TRUE;
            }
        ...
        case (CLaygo::lgoVacuumPaths):
            if (m_ProbabilityGrid->GetPercentExploredArea() >= 1)
            {
                Show =  TRUE;
            }
        ...
        case (CLaygo::lgoDangerZones):
            if (m_ProbabilityGrid->GetPercentExploredArea() >= 1.5)
            {
                Show =  TRUE;
            }
        ...
        case (CLaygo::lgoFlowZap):
            if (m_ProbabilityGrid->m_RWGraphicObjectList->NumberOfNodes() > 6)
            {
                Show =  TRUE;
            }
...
void CRobotWorldView::OnLButtonDblClk(UINT nFlags, CPoint point)
{
    if (m_CurrentLinePath != NULL)
    {
        if (m_MouseMode == mmPlaceLinePath)
        {
            ...
            if (m_LinePathTo == m_CurrentLinePath)
            {
                m_LinePathTo = NULL;
            }
...
void CRobotWorldView::SetVacuumAttached(bool attached)
{
    ...
    if (Robot != NULL)
    {
        if (attached)
        {
            Robot->SetAttachmentType(atVacuum);
        }
        else
        {
            Robot->SetAttachmentType(atNone);
        }
    }
}
...
void CRobotWorldView::SetShowVelocityTorqueDisplay(bool Show)
{
    ...
    else
    {
        ...
        {
            m_DlgVelocityTorqueDisplay->ShowWindow(SW_HIDE);
        }
    }
}
...
void CRobotWorldView::OnMotorCurrentUpdate(float RightMotor, float LeftMotor, CRobot* Robot)
{
    if (Robot == GetActiveRobot())
    {
        if (m_DlgVelocityTorqueDisplay != NULL)
        {
            m_DlgVelocityTorqueDisplay->SetTorque(LeftMotor, RightMotor);
        }
    }
}
...
void CRobotWorldView::OnMotorVelocityUpdate(short RightVelocity, short LeftVelocity, CRobot* Robot)
{
    if (Robot == GetActiveRobot())
    {
        if (m_DlgVelocityTorqueDisplay != NULL)
        {
            m_DlgVelocityTorqueDisplay->SetVelocity(LeftVelocity, RightVelocity);
        }
    }
}
