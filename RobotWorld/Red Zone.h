...
class CRedZone : public CRWGraphicObject
{
    ...
    private:
        ...
        unsigned int HorizontalGridSize(void)
        {
            return unsigned int((m_Bounds.right - m_Bounds.left) / cWorldGridPointSize + 1);
        };
        unsigned int VerticalGridSize(void)
        {
            return unsigned int((m_Bounds.top - m_Bounds.bottom) / cWorldGridPointSize + 1);
        };
        CRedZone();
