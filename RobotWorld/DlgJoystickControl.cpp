...
*******************************************************************************/
{
    if (JoyControlType == jcRobotPwm)
    {
        JoyControlType = jcRobotVelocity;
    }
    else if (JoyControlType == jcWorldPwm)
    {
        JoyControlType = jcWorldVelocity;
    }
...
void CDlgJoystickControl::SetResponsiveness(float Responsiveness)
{
    ...
    if (m_hWnd != NULL)
    {
        m_sliResponsiveness.SetPos(2 - (int)m_Responsiveness);
    }
}
...
void CDlgJoystickControl::SetTurnGain(float Gain)
{
    ...
    if (m_hWnd != NULL)
    {
        m_sliTurnGain.SetPos(int(cTurnGainRange + 1 - m_TurnGain));
    }
}
...
*******************************************************************************/
{
    ...
    if (m_hWnd != NULL)
    {
        if (Enable)
        {
            ...
            if (m_RobotCoordinates)
            {
                m_sliTurnGain.EnableWindow(TRUE);
            }
        }
        else
        {
