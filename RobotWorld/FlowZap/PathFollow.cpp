 ...
 **********************************************************************/
  ...
  void CPathFollow::MakeCondensed()
  {
  	...
  	m_PathList.ShowWindow(SW_HIDE);		m_DisabledImage.ShowWindow(SW_HIDE);		
  	m_SpeedSlider.ShowWindow(SW_HIDE);	m_Image.ShowWindow(SW_HIDE);
  	m_Text1.ShowWindow(SW_HIDE);		m_Text2.ShowWindow(SW_HIDE);
  ...
  void CPathFollow::RemoveLabel(CString OldCaption)
  {
  	...
  	if (result != CB_ERR)m_PathList.DeleteString(result);
  
  	if (currentValue == OldCaption) m_PathList.SelectString (-1, "[CHOOSE PATH]");
  }
