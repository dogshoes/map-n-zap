...
void CPallette::OnPalletteItemClicked(UINT nFlags, CPalletteItem* PalletteItem)
{
    ...
    {
        if (PalletteItem == &m_butWait)
        {
            ((CFlowZap *)m_Parent)->m_MouseMode = mmFZWait;
        }
        else if (PalletteItem == &m_butRepeat)
        {
            ((CFlowZap *)m_Parent)->m_MouseMode = mmFZRepeat;
        }
        else if (PalletteItem == &m_butBeep)
        {
            ((CFlowZap *)m_Parent)->m_MouseMode = mmFZBeep;
        }
        else if (PalletteItem == &m_butStop)
        {
            ((CFlowZap *)m_Parent)->m_MouseMode = mmFZStop;
        }
        else if (PalletteItem == &m_butBranch)
        {
            ((CFlowZap *)m_Parent)->m_MouseMode = mmFZBranch;
        }
        else if (PalletteItem == &m_butHotSpot)
        {
            ((CFlowZap *)m_Parent)->m_MouseMode = mmFZGoto;
        }
        else if (PalletteItem == &m_butSubroutine)
        {
            ((CFlowZap *)m_Parent)->m_MouseMode = mmFZSub;
        }
        else if (PalletteItem == &m_butVacuum)
        {
            ((CFlowZap *)m_Parent)->m_MouseMode = mmFZVac;
        }
        else if (PalletteItem == &m_butCalibrate)
        {
            ((CFlowZap *)m_Parent)->m_MouseMode = mmFZCal;
        }
        else if (PalletteItem == &m_butPath)
        {
            ((CFlowZap *)m_Parent)->m_MouseMode = mmFZPath;
        }
        else if (PalletteItem == &m_butMotion)
        {
            ((CFlowZap *)m_Parent)->m_MouseMode = mmFZMotion;
        }
        else if (PalletteItem == &m_butExternalOutput)
        {
            ((CFlowZap *)m_Parent)->m_MouseMode = mmFZExternalOutput;
        }
