 ...
 **********************************************************************/
  ...
  BOOL CWait::OnInitDialog() 
  {
  	...
  	m_TimeLength.AddString ("1/4"); m_TimeLength.AddString ("1/2"); m_TimeLength.AddString ("3/4");
  	for (int i=1; i<=60; i++) m_TimeLength.AddString (IntToString(i) );
  ...
  void CWait::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
  {
  	...
  	if (m_ChangeTimePos > (int)nPos) {
  		//clicked down arrow
  		if (EditBoxNum <= minValue) EditBoxNum = maxValue;
  		else EditBoxNum --;
  		
  	}
  	else {
  		//clicked up arrow
  		if (EditBoxNum >= maxValue) EditBoxNum = minValue;
  		else EditBoxNum ++;
  	}
  	...
  	if (m_CurrentEditBox == dEDIT_MIN){
  		if (EditBoxNum < 10) sEditBoxVal = "0" + sEditBoxVal;
  	}
  ...
  void CWait::OnUpdateWaitMin() 
  {
  	...
  	int min = atoi (val);
  	if (min > 59) m_WaitMin.SetWindowText("59");
  	if (min < 0)  m_WaitMin.SetWindowText("00");
  ...
  void CWait::OnUpdateWaitHour() 
  {
  	...
  	int hour = atoi (val);
  	if (hour > 12) m_WaitHour.SetWindowText("12");
  	if (hour < 1)  m_WaitHour.SetWindowText("1");
  ...
  //	switch (index) {
  	...
  	case wcWaitUntil:
  		...
  		/*Make sure hour and minutes are valid*/
  			if ( (dHours > 12) || (dMinutes >= 60) || (dHours <= 0) || (dMinutes < 0) ) return TR_Error;
  
  			if (m_am_pm.GetCurSel() == 0) {
  				if (dHours == 12) dHours = 0;
  			} else {
  				if (dHours != 12) dHours += 12;
  			}
  	...
  	case wcWaitFor:
  		...
  		/*Minutes must be < 1440 (minutes in 1 day) or else the wait until instruction screws up*/
  			...
  			if (minutes > 1439.0f) return TR_Error;
