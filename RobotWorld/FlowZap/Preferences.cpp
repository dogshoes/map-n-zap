...
void CPreferences::OnOK()
{
    ...
    if (m_CondensedView.GetCheck() == 0)
    {
        m_TestCondensedView = FALSE;
    }
    else
    {
        m_TestCondensedView = TRUE;
    }

    if (m_lastProject.GetCheck() == 0)
    {
        m_TestLastProject = FALSE;
    }
    else
    {
        m_TestLastProject = TRUE;
    }

    if (m_SoundOn.GetCheck() == 0)
    {
        m_TestSoundOn = FALSE;
    }
    else
...
void CPreferences::ReadPrefsFromFile(CStdioFile *prefs)
{
    ...
    prefs->ReadString(last);
    m_TestLastProject =		StringToBool(last);
    prefs->ReadString(m_PreviousFileName);
    prefs->ReadString(condensed);
    m_TestCondensedView =	StringToBool(condensed);
    prefs->ReadString(sounds);
    m_TestSoundOn =		StringToBool(sounds);
...
void CPreferences::OnShowWindow(BOOL bShow, UINT nStatus)
{
    ...
    //	ReadPrefs();
    if (m_TestLastProject)
    {
        m_lastProject.SetCheck(1);
    }
    else
    {
        m_lastProject.SetCheck(0);
    }

    if (m_TestCondensedView)
    {
        m_CondensedView.SetCheck(1);
    }
    else
    {
        m_CondensedView.SetCheck(0);
    }


    if (m_TestSoundOn)
    {
        m_SoundOn.SetCheck(1);
    }
    else
    {
        m_SoundOn.SetCheck(0);
    }
