...
void CLeftLoopPanel::OnMouseMove(UINT nFlags, POINT p)
{
    ...
    if (m_LoopPtr == NULL)
    {
        return;    // If there is no loop selected
    }
    ...
    if (in->IsKindOf(RUNTIME_CLASS(CRepeat)) && in != l->m_Start)
    {
        l->m_LegalPoint = FALSE;
    }

    if (in->GetIndex() > l->m_Start->GetIndex())
    {
        in = l->m_Start;
    }
    ...
    for (int i = 0; i < fc->GetInstructionsSize() - 1; i++)
    {
        ...
        if (in->IsKindOf(RUNTIME_CLASS(CRepeat)))
        {
            ...
            for (int j = endIndex + 1; j < startIndex; j++)
            {
                ...
                if (changing->IsKindOf(RUNTIME_CLASS(CRepeat)))
                {
                    changing->m_Loop->m_Level++;
                }
            }
        }
