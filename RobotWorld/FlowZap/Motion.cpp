...
void CMotion::MakeCondensed()
{
    ...
    m_TextGo.ShowWindow(SW_HIDE);
    m_Direction.ShowWindow(SW_HIDE);
    m_TextOClock.ShowWindow(SW_HIDE);
    m_Motion.ShowWindow(SW_HIDE);
...
void CMotion::MakeExpanded()
{
    ...
    m_TextGo.ShowWindow(SW_SHOW);
    m_Direction.ShowWindow(SW_SHOW);
    m_TextOClock.ShowWindow(SW_SHOW);
    m_Motion.ShowWindow(SW_SHOW);
...
BOOL CMotion::OnInitDialog()
{
    ...
    for (int i = 1; i <= 12; i++)
    {
        m_Degrees.AddString(IntToString(i));
    }
...
TTaskResult CMotion::Execute(CRobot * Robot, CExecutableRWGraphicObjectList * ExecutableObjectList, bool & result)
{
    ...
    if (m_Motion.GetCheck())
    {
        ...
        if (Distance < 0)
        {
            Distance -= Robot->GetHandleLength();
        }
        else
        {
            Distance += Robot->GetHandleLength();
