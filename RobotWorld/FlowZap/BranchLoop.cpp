...
void CBranchLoop::CalculateLocation(CDialog * parent, int index, int maxIndex, int w)
{
    ...
    */
    ...
    if (y1 > r.Height() / 2 - 1 && y1 < r.Height() / 2 + 1)
    {
        y1 -= 7;    // KEEP IT OFF THE CENTER
    }
    ...
    m_ArrowVertices[0].x = r.right;
    m_ArrowVertices[0].y = y1;
    m_ArrowVertices[1].x = r.right + 12;
    m_ArrowVertices[1].y = y1 - 3;
    m_ArrowVertices[2].x = r.right + 12;
    m_ArrowVertices[2].y = y1 + 3;
}
