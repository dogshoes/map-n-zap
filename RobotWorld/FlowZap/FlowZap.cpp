...
BOOL CFlowZap::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
    if (nHitTest == HTCLIENT && nHitTest != HTVSCROLL && nHitTest != HTHSCROLL)
    {
        switch (m_MouseMode)
        {
        case mmFZStop:		::SetCursor(::LoadCursor(GetModuleHandle(NULL), MAKEINTRESOURCE(IDC_STOP)));
            return true;
            break;

        case mmFZBeep:		::SetCursor(::LoadCursor(GetModuleHandle(NULL), MAKEINTRESOURCE(IDC_BEEP)));
            return true;
            break;

        case mmFZVac:		::SetCursor(::LoadCursor(GetModuleHandle(NULL), MAKEINTRESOURCE(IDC_VACUUM)));
            return true;
            break;

        case mmFZCal:		::SetCursor(::LoadCursor(GetModuleHandle(NULL), MAKEINTRESOURCE(IDC_CAL)));
            return true;
            break;

        case mmFZRepeat:	::SetCursor(::LoadCursor(GetModuleHandle(NULL), MAKEINTRESOURCE(IDC_REPEAT)));
            return true;
            break;

        case mmFZWait:		::SetCursor(::LoadCursor(GetModuleHandle(NULL), MAKEINTRESOURCE(IDC_WAIT_CURSOR)));
            return true;
            break;

        case mmFZGoto:		::SetCursor(::LoadCursor(GetModuleHandle(NULL), MAKEINTRESOURCE(IDC_GOTO)));
            return true;
            break;

        case mmFZBranch:	::SetCursor(::LoadCursor(GetModuleHandle(NULL), MAKEINTRESOURCE(IDC_BRANCH)));
            return true;
            break;

        case mmFZSub:		::SetCursor(::LoadCursor(GetModuleHandle(NULL), MAKEINTRESOURCE(IDC_SUB)));
            return true;
            break;

        case mmFZMotion:	::SetCursor(::LoadCursor(GetModuleHandle(NULL), MAKEINTRESOURCE(IDC_MOTION)));
            return true;
            break;

        case mmFZExternalOutput:	::SetCursor(::LoadCursor(GetModuleHandle(NULL), MAKEINTRESOURCE(IDC_EXTERNAL_OUTPUT)));
            return true;
            break;

        case mmFZMove:	::SetCursor(::LoadCursor(GetModuleHandle(NULL), MAKEINTRESOURCE(IDC_MOVE)));
            return true;
            break;

        case mmFZPath:	::SetCursor(::LoadCursor(GetModuleHandle(NULL), MAKEINTRESOURCE(IDC_PATH)));
            return true;

        case mmFZMoveRepeatLoop:	::SetCursor(::LoadCursor(GetModuleHandle(NULL), MAKEINTRESOURCE(IDC_MOVE)));
            return true;
            break;

        case mmFZMoveBranchLoop:	::SetCursor(::LoadCursor(GetModuleHandle(NULL), MAKEINTRESOURCE(IDC_MOVE)));
            return true;
            break;

        case mmFZMoveCue:			::SetCursor(::LoadCursor(GetModuleHandle(NULL), MAKEINTRESOURCE(IDC_MOVE)));
            return true;
            break;

        case mmFZDefault: ::SetCursor(::LoadCursor(NULL, IDC_ARROW));
            return true;
            break;
...
void CFlowZap::OnMouseMove(UINT nFlags, CPoint point)
{
    ...
    ************************************************************************************************/
    ...
    else if (m_MouseMode == mmFZMoveCue)          /*THEN*/
    {
        m_Cue->OnMouseMove(nFlags, point);
    }
    else if (m_MouseMode == mmFZMoveRepeatLoop)   /*THEN*/
    {
        m_llp->OnMouseMove(nFlags, point);
    }
    else if (m_MouseMode == mmFZMoveBranchLoop)   /*THEN*/
    {
        m_rlp->OnMouseMove(nFlags, point);
    }
...
void CFlowZap::OnLButtonUp(UINT nFlags, CPoint point)
{
    ...
    ************************************************************************************************/
    ...
    else if (m_MouseMode >= mmFZVac && m_MouseMode <= mmFZGoto)
    {
        ...
        {
            AddInstruction(m_MouseMode, ptr);
        }
...
void CFlowZap::AddInstruction(TFZMouseMode mode, CInstruction* ptr)
{
    ...
    case mmFZExternalOutput	:
        in = makeExternalOutput(index);
        break;

    case mmFZPath	:
        in = makePathFollow(index);
        break;
    }
...
void CFlowZap::InsertInstructionAfter(CInstruction* bench, CInstruction* insert)
{
    ...
    ************************************************************************************************/
    if (bench == NULL)
    {
        bench = GetInstructionAtIndex(GetInstructionsSize() - 2);
    }

    if (bench == GetInstructionAtIndex(GetInstructionsSize() - 1))
    {
        bench = GetInstructionAtIndex(GetInstructionsSize() - 2);
    }
...
void CFlowZap::InsertInstructionBefore(CInstruction * bench, CInstruction * insert)
{
    ...
    ************************************************************************************************/
    ...
    if (bench == GetInstructionAtIndex(0))
    {
        bench = GetInstructionAtIndex(1);
    }
...
CInstruction * CFlowZap::makeVac(int index)
{
    ...
    if (m_VacuumPatches != NULL)
    {
        newVac->InsertVacuumPatches(m_VacuumPatches);
    }
...
CInstruction* CFlowZap::makePathFollow(int index)
{
    ...
    if (m_LinePaths != NULL)
    {
        newPath->InsertLinePaths(m_LinePaths);
    }
...
CInstruction* CFlowZap::makeGoto(int index)
{
    ...
    if (m_LabelNames != NULL)
    {
        newGoto->InsertLabelNames(m_LabelNames);
    }
...
CInstruction * CFlowZap::makeCal(int index)
{
    ...
    if (m_CalibrationZones != NULL)
    {
        newCalibrate->InsertCalibrationZones(m_CalibrationZones);
    }
...
BOOL CFlowZap::OnInitDialog()
{
    ...
    if (m_PreferencesControl->m_TestCondensedView)
    {
...
void CFlowZap::UpdateZonePlacements()
{
    ...
    if (GetInstructionAtIndex(0) != NULL)
    {
        GetInstructionAtIndex(0)->GetClientRect(&childRect);
    }
...
int CFlowZap::InitInstructionList()
{
    ...
    ************************************************************************************************/
    ...
    // START								// STOP
    CInstruction *start = makeStart(0);
    CInstruction *stop  = makeStop(1);
    start->ShowWindow(SW_SHOW);
    stop->ShowWindow(SW_SHOW);
    m_instructions.AddHead(start);
    m_instructions.AddTail(stop);
    start->MakeMoveable(FALSE);
    stop->MakeMoveable(FALSE);
...
void CFlowZap::PaintArrows(CPaintDC *dc)
{
    ...
    for (int i = 0; i < m_arrows.GetCount(); i ++)
    {
        ...
        if (ar == m_hArrow)
        {
            ar->PaintArrow(COLORREF(0xff), dc);    // red
        }
        else
        {
            ar->PaintArrow(COLORREF(0x000000), dc);    // black
        }
    }
...
void CFlowZap::OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags)
{
    if (m_SelectedInstruction != NULL)
    {
        RemoveInstruction(m_SelectedInstruction);
    }
...
CInstruction* CFlowZap::MakeRepeatFromFile(CStringList * words, int index)
{
    ...
    else if (pointingTo == index)
    {
        ptr = in;
    }
    else
    {
        ptr = GetInstructionAtIndex(pointingTo);
    }
...
void CFlowZap::MakeNewInstructionFromFile(CStringList * words, int index)
{
    ...
    {
        in = MakeGotoFromFile(words);
    }
    else if (head == "Repeat")
    {
        in = MakeRepeatFromFile(words, index);
    }
    else if (head == "If")
    {
        in = MakeBranchFromFile(words);
    }
    else if (head == "Wait")
    {
        in = MakeWaitFromFile(words);
    }
    else if (head == "Vacuum")
    {
        in = MakeVacuumFromFile(words);
    }
    else if (head == "Calibrate")
    {
        in = MakeCalibrateFromFile(words);
    }
    else if (head == "Motion:")
    {
        in = MakeMotionFromFile(words);
    }
    else if (head == "External")
    {
        in = MakeExternalOutputFromFile(words);
    }
    else if (head == "Turn")
    {
        in = MakeExternalOutputFromFile(words);
...
void CFlowZap::WriteInstructionsToFile(CStdioFile * f)
{
    ...
    try
    {
        ...
        if (m_ExpandedView)
        {
            f->WriteString("EXPANDED VIEW\n");
        }
        else
        {
            f->WriteString("CONDENSED VIEW\n");
        }
...
CInstruction* CFlowZap::RemoveInstruction(CInstruction *in)
{
    ...
    for (int i = 0; i < GetInstructionsSize(); i++)
    {
        ...
        {
            if (ptr->m_Loop->m_End == in)
            {
                ptr->m_Loop->m_End = ptr;    //point it to itself
            }
        }
...
CInstruction* CFlowZap::DeselectInstruction()
{
    ...
    if (m_SelectedInstruction != NULL && m_SelectedInstruction->GetSafeHwnd() != NULL)
    {
        ...
        if (!m_ExpandedView)
        {
            m_SelectedInstruction->MakeCondensed();
        }
...
void CFlowZap::OnInstructionDisable()
{
    if (m_SelectedInstruction != NULL)
    {
        EnableInstruction(m_SelectedInstruction, FALSE);
    }
...
void CFlowZap::OnInstructionEnable()
{
    if (m_SelectedInstruction != NULL)
    {
        EnableInstruction(m_SelectedInstruction, TRUE);
    }
...
void CFlowZap::EnableInstruction(CInstruction* in, BOOL enable)
{
    if (in != NULL)
    {
        ...
        if (in->IsKindOf(RUNTIME_CLASS(CBranch)))
        {
            RedrawWindow(m_rlp->m_Loc, NULL, RDW_INVALIDATE | RDW_NOCHILDREN | RDW_ERASE);
        }
    }
...
{
    m_NoteConversion [0] = "low C for";
    m_NoteConversion [1] = "low C# for";
    m_NoteConversion [2] = "low D for";
    m_NoteConversion [3] = "low D# for";
    m_NoteConversion [4] = "low E for";
    m_NoteConversion [5] = "low F for";
    m_NoteConversion [6] = "low F# for";
    m_NoteConversion [7] = "low G for";
    m_NoteConversion [8] = "low G# for";
    m_NoteConversion [9] = "low A for";
    m_NoteConversion [10] = "low A# for";
    m_NoteConversion [11] = "low B for";

    m_NoteConversion [12] = "mid C for";
    m_NoteConversion [13] = "mid C# for";
    m_NoteConversion [14] = "mid D for";
    m_NoteConversion [15] = "mid D# for";
    m_NoteConversion [16] = "mid E for";
    m_NoteConversion [17] = "mid F for";
    m_NoteConversion [18] = "mid F# for";
    m_NoteConversion [19] = "mid G for";
    m_NoteConversion [20] = "mid G# for";
    m_NoteConversion [21] = "mid A for";
    m_NoteConversion [22] = "mid A# for";
    m_NoteConversion [23] = "mid B for";

    m_NoteConversion [24] = "high C for";
    m_NoteConversion [25] = "high C# for";
    m_NoteConversion [26] = "high D for";
    m_NoteConversion [27] = "high D# for";
    m_NoteConversion [28] = "high E for";
    m_NoteConversion [29] = "high F for";
    m_NoteConversion [30] = "high F# for";
    m_NoteConversion [31] = "high G for";
    m_NoteConversion [32] = "high G# for";
    m_NoteConversion [33] = "high A for";
    m_NoteConversion [34] = "high A# for";
    m_NoteConversion [35] = "high B for";
...
*/
POSITION CFlowZap::GetInstructionPosition(CInstruction * in)
{
    return m_instructions.Find(in);
}
...
*/
int CFlowZap::GetInstructionsSize()
{
    return m_instructions.GetCount();
}
...
CInstruction* CFlowZap::GetInstructionAtIndex(int index)
{
    ...
    if (index >= m_instructions.GetCount())
    {
        return NULL;
    }
...
void CFlowZap::SwitchToExecuteMode(BOOL execute)
{
    if (execute)
    {
        m_ProgramMode = pmExecute;
    }
    else
    {
        m_ProgramMode = pmEdit;
    }
}
...
CInstruction* CFlowZap::GetNextInstructionInList(CInstruction * in)
{
    ...
    ************************************************************************************************/
    ...
    if (index >= GetInstructionsSize() - 1)
    {
        in = in;
    }
    else
    {
        in =  GetInstructionAtIndex(index + 1);
    }
...
void CFlowZap::HandlePopupFileMenu(CMenu* menu)
{
    ...
    */
    ...
    {
        menu->EnableMenuItem(ID_POPUP_CLOSE, MF_BYCOMMAND | MF_GRAYED);
    }
    else
    {
        menu->EnableMenuItem(ID_POPUP_CLOSE, MF_BYCOMMAND | MF_ENABLED);
    }
...
void CFlowZap::OnPopupDelete()
{
    ...
    ************************************************************************************************/
    ...
    if (in == m_instructions.GetHead() || in == m_instructions.GetTail())
    {
        return;
    }
...
void CFlowZap::OnPlayStart()
{
    ...
    {
        if (m_RobotWorldView->GetActiveRobot()->GetState() != stWaiting)
        {
            return;
        }
    }
