...
ExitLoop:
    ...
    if (nHandle >= 0)
    {
        ...
        if ((temp.x != pointSave.x) || (temp.y != pointSave.y))
        {
            return TRUE;
        }
    }
    ...
    {
        if ((point.x != pointSave.x) || (point.y != pointSave.y))
        {
            return TRUE;
        }
    }
...
int CPolyLineTracker::HitTestHandles(CPoint point)
{
    ...
    while (Position != NULL)
    {
        ...
        else
        {
            ...
            {
                if (PerpendicularDistanceFromLine(LineSegment, point) <= m_nHandleSize / 2.0)
                {
                    return htLineSegment;
                }
            }
        }
