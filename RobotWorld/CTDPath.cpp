  ...
  *******************************************************************************/
  {
  	...
  	if (ThetaStart < 0) ThetaStart += 2*pi;
  	ThetaEnd = atan2(End.y - Center.y, End.x - Center.x);
  	if (ThetaEnd < 0) ThetaEnd += 2*pi;
  	Theta3 = atan2(OnArc.y - Center.y, OnArc.x - Center.x);
  	if (Theta3 < 0) Theta3 += 2*pi;
  	double PhiEnd, Phi3;
  	PhiEnd = ThetaEnd - ThetaStart;
  	if (PhiEnd < 0) PhiEnd += 2*pi;
  	Phi3 = Theta3 - ThetaStart;
  	if (Phi3 < 0) Phi3 += 2*pi;
  /*Find which way to rotate*/
  	bool PositiveAngularRotation;
  	if (Phi3 < PhiEnd) PositiveAngularRotation = true;
  	else PositiveAngularRotation = false;
  /*Find the length of the arc*/
  	double ArcLength;
  	if (PositiveAngularRotation) ArcLength = Radius * PhiEnd;
  	else ArcLength = Radius * (2*pi - PhiEnd);
  ...
  *******************************************************************************/
  ...
  CCTDPath::CCTDPath(unsigned int Speed)
  {
  	...
  	if (Speed < 125) PathSpeed = ps100;
  	else if (Speed < 175) PathSpeed = ps150;
  	else if (Speed < 225) PathSpeed = ps200;
  	else if (Speed < 275) PathSpeed = ps250;
  	else PathSpeed = ps300;
  ...
  *******************************************************************************/
  ...
  unsigned short CCTDPath::GetSpeed(void) {return m_Speed;};
  ...
  /*Take care of empty, 1 point path*/
  	if (m_Size <= StartPosition) return m_Size;
  
  	MinDist = Distance(m_Path[StartPosition], Point);
  	ClosestPosition = StartPosition;
  
  	if ( (((long)StartPosition) + NumPositionsToSearch) > m_Size ) NumPositionsToSearch = m_Size - StartPosition;
  ...
  bool CCTDPath::operator ==(CCTDPath P2)
  {
  	if (P2.m_Size != m_Size) return false;
  	for (unsigned short i = 0; i < m_Size; i++) {
  		if ( m_Path[i] != P2.m_Path[i] ) return false;
  	}
  	return true;
  }
