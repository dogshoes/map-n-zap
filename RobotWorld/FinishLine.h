 ...
 **********************************************************************/
 ...
 public:
 	...
 	int GetNumberAnimationFrames(void) {return 10;};
 	...
 	unsigned int GetZLayer(void) {return 100;};
 	...
 	TRWGraphicObjectType GetObjectType(void) {return goFinishLine;};
 	void Draw(CRect ProbGridBmpUpdateRect, CDC* pDC, CProbabilityGrid* ProbabilityGrid);
 	CFinishLine();
 	virtual ~CFinishLine();
 	void SetTimerPosition(short Position) {(Position >= 3) ? (m_TimerPosition = 3) : (m_TimerPosition = Position);};
 	short GetTimerPosition(void) {return m_TimerPosition;};
 protected:
