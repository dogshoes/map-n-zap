...
void CExecutableRWGraphicObject::SetEntryPoint(robPOINT Point)
{
    ...
    {
        if ((m_EntryPoint.x != Point.x) || (m_EntryPoint.y != Point.y))
        {
            m_ExecutableObjectList->RemoveFromStoredPaths(m_EntryPoint);
        }
    }
...
{
    if (CRWGraphicObject::InsideObject(point, WorldPointsPerPixel))
    {
        return true;
    }
