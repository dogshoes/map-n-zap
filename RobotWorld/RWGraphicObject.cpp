...
void CRWGraphicObjectList::Serialize(CArchive & archive)
{
    ...
    else
    {
        ...
        char temp;
        archive >> temp;
        m_ShowCalibrationZones = temp != 0;
        archive >> temp;
        m_ShowPositionLabels = temp != 0;
        archive >> temp;
        m_ShowRedZones = temp != 0;
        archive >> temp;
        m_ShowVacuumPatches = ((temp & 0x01) == 0x01);
        m_ShowLinePaths = ((temp & 0x02) == 0x02);
        m_ShowCalibrationZones = true;
...
CRectTracker CRWGraphicObject::SetupTracker(CProbabilityGrid* ProbabilityGrid, CPoint SurfaceOffset)
{
    ...
    if (ClientPoint.x > Tracker.m_sizeMin.cx)
    {
        Tracker.m_sizeMin.cx = ClientPoint.x;
    }

    if (ClientPoint.y > Tracker.m_sizeMin.cy)
    {
        Tracker.m_sizeMin.cy = ClientPoint.y;
    }

    /*Set handle size*/
    const short cHandleSize = 7;

    if (Tracker.m_nHandleSize < cHandleSize)
    {
        Tracker.m_nHandleSize = cHandleSize;
    }
...
{
    if ((m_Animate == true) && (Animate == false))
    {
        Redraw(true);
    }
