...
BOOL CLaygo::OnInitDialog()
{
    ...
    if (m_CurrentState == lgoHomeBaseCreateState)
    {
        Configure(m_CurrentState);
    }
...
void CLaygo::LoadState()
{
    ...
    if (LaygoStep == lgoComPortSearch)
    {
        LaygoStep--;
    }

    if (LaygoStep > cNonlinearLaygoes)
    {
        LaygoStep = cNonlinearLaygoes;
    }
    else if (LaygoStep < 0)
    {
        LaygoStep = 0;
